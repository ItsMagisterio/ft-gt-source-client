package alternativa.tanks.service.settings.keybinding
{
   ...
   public class KeysBindingServiceImpl extends EventDispatcher implements KeysBindingService
   {
      ...
      private var bindingsKeyToAction:Dictionary;

      private var bindingsActionToKey:Dictionary;

      private var defaultKeyLeft:uint;

      private var defaultBindings:Dictionary;
      ...
      public function KeysBindingServiceImpl()
      {
         ...
         this.defaultBindings = new Dictionary();
         super();
         var _loc1_:ILocaleService = ILocaleService(_loc1_);
         this.storageService = IStorageService(this.storageService);
         this.defaultKeyLeft = Keyboard.Z;
      ...
      private function getKeyCodesConverter():alternativa.tanks.service.settings.keybinding.KeyCodesConverter
      {
         if (!this.keyCodesConverter)
         {
            this.keyCodesConverter = new alternativa.tanks.service.settings.keybinding.KeyCodesConverter();
         }
      ...
      public function setKeyBinding(gameAction:GameActionEnum, keyCode:uint, keyIndex:int):Boolean
      {
         var _loc5_:GameActionEnum = null;
         var _loc6_:Vector.<uint> = null;
         var _loc7_:int = 0;
         if (keyIndex >= MAX_BINDING_KEY_ON_ACTION)
         {
            return false;
         }
         if (!this.isFreeKey(keyCode))
         {
            _loc5_ = this.bindingsKeyToAction[keyCode];
            _loc6_ = this.bindingsActionToKey[_loc5_];
            _loc7_ = _loc6_.indexOf(keyCode);
            delete this.bindingsKeyToAction[keyCode];
         ...
         var _loc4_:uint = this.getKeyBinding(gameAction, keyIndex);
         if (!isNullKeyCode(_loc4_) && _loc4_ != keyCode)
         {
            delete this.bindingsKeyToAction[_loc4_];
         }
         this.bindingsKeyToAction[keyCode] = gameAction;
         this.bindingsActionToKey[gameAction][keyIndex] = keyCode;
         this.storeKeyBinding(gameAction, this.bindingsActionToKey[gameAction]);
         return true;
      }
      ...
      private function storeKeyBinding(param1:GameActionEnum, param2:Vector.<uint> = null):void
      {
         ...
         var _loc3_:SharedObject = SharedObject.getLocal("keyBindings");
      ...
      public function restoreDefaultBindings():void
      {
         ...
         this.bindingsActionToKey = new Dictionary();
         this.bindingsKeyToAction = new Dictionary();
         for (_loc1_ in this.defaultBindings)
         {
      ...
      private function restoreBindingsFromStorage():void
      {
         ...
         var _loc1_:SharedObject = SharedObject.getLocal("keyBindings");
