package alternativa.tanks.service.item
{
   ...
   public class ItemServiceImpl extends EventDispatcher implements alternativa.tanks.service.item.ItemService
   {
      ...
      [Inject]
      public static var itemService:alternativa.tanks.service.item.ItemService;

      [Inject]
      public static var resistancesService:ResistanceService;

      private var mountedItems:Vector.<IGameObject>;

      private var itemToModifications:Dictionary;

      private var hasItemMap:Dictionary;

      private var groupedItems:Dictionary;

      private var isMountableCategoryMap:Dictionary;

      private var resistanceModuleDescription:Dictionary;

      private const delayItemAppearingInSeconds:int = 5;

      public function ItemServiceImpl()
      {
         this.mountedItems = new Vector.<IGameObject>();
         this.groupedItems = new Dictionary();
         this.resistanceModuleDescription = new Dictionary();
         super();
         this.reset();
      ...
      {
         var _loc2_:String = String(IDescription(param1.adapt(IDescription)).getName());
         if (itemService.isModificationItem(param1))
         {
            _loc2_ += (localeService.language == LocaleServiceLangValues.CN ? "" : " ") + itemService.getModificationIndexText(param1);
         }
         return _loc2_;
      }
      ...
      public function isMaxModification(param1:IGameObject):Boolean
      {
         return IModification(param1.adapt(IModification)).getModificationIndex() == 6;
      }

      public function getModificationIndexText(param1:IGameObject):String
      {
         return "Mk" + (this.getModificationIndex(param1) + 1);
      }
      ...
      public function getMaxAvailableModification(param1:IGameObject):IGameObject
      {
         ...
         if (userPropertyService.rank < this.getMinRankIndex(param1) || this.isMaxModification(param1))
         {
            return param1;
         }
         var _loc2_:Vector.<IGameObject> = this.getModifications(param1);
         ...
         return userPropertyService.rank >= this.getMinRankIndex(_loc2_[0]) ? _loc2_[0] : null;
      }
      ...
      public function isSellingForCoin(param1:IGameObject):Boolean
      {
         return IBuyable(param1.adapt(IBuyable)).isSellingForCoin();
      }

      public function isSellingForCrystal(param1:IGameObject):Boolean
      {
         return IBuyable(param1.adapt(IBuyable)).isSellingForCrystal();
      }
   }
}
