package alternativa.tanks.gui.communication.tabs.news
{
	...
	public class NewsTab extends AbstractCommunicationPanelTab
	{
		...
		private var newsId2NewsItem:Dictionary;
		...
		public function NewsTab()
		{
			...
			this.newsId2NewsItem = new Dictionary();
			super();
			...
			// (Main.osgi.getService(NewsService) as NewsServiceImpl).setNewsAddingCallback(this.addNewsItems);
			// (Main.osgi.getService(NewsService) as NewsServiceImpl).addEventListener(NewsServiceEvent.NEWS_ITEM_IS_SENT,this.onNewsItemAdded);
			// (Main.osgi.getService(NewsService) as NewsServiceImpl).addEventListener(NewsServiceEvent.NEWS_ITEM_IS_REMOVED,this.onNewsItemRemoved);
		...
		private function onNewsItemAdded(param1:NewsServiceEvent):void
		{
			...
			var _loc5_:String = _loc2_.dateInSeconds;
		...
		public function addNewsItems(param1:Vector.<NewsItemData>):void
		{
			...
			this.clearAll();
			var _loc2_:String = "";
			var _nado:Boolean = true;
			for each (_loc3_ in param1)
			{
				_loc4_ = this.createNewsItem(_loc3_);
				if (_loc4_ != null)
				{
					_loc5_ = _loc3_.dateInSeconds;

					if (IStorageService(Main.osgi.getService(IStorageService)).getStorage().data["NEWS_VIEWED_" + _loc3_.id] == undefined && _nado)
					{
						_nado = false;
		...
		private function clearAll():void
		{
			...
			this.scrollPane.source = this.scrollContainer;
			this.scrollPane.update();
			(Main.osgi.getService(NewsService) as NewsServiceImpl).clearExpiredReadNews();
		}

		public function destroy():void
		{
			var _loc2_:NewsItem = null;
			(Main.osgi.getService(NewsService) as NewsServiceImpl).resetNewsAddingCallback();
			this.scrollPane.removeEventListener(MouseEvent.MOUSE_WHEEL, this.onMouseWheel);
			(Main.osgi.getService(NewsService) as NewsServiceImpl).removeEventListener(NewsServiceEvent.NEWS_ITEM_IS_SENT, this.onNewsItemAdded);
			(Main.osgi.getService(NewsService) as NewsServiceImpl).removeEventListener(NewsServiceEvent.NEWS_ITEM_IS_REMOVED, this.onNewsItemRemoved);
