package alternativa.tanks.models.battlefield
{
    ...
    import logic.test.spectator.SpectatorCameraController;
    ...
    import logic.test.anticheat.SpeedHackChecker;
    import logic.test.RenderSystem;
    ...
    import alternativa.tanks.input.GameActionListener;
    ...
    public class BattlefieldModel extends BattlefieldModelBase implements IBattlefieldModelBase, IBattleField, IObjectLoadListener, IBonusListener, INetworkListener, IPanelListener, IBodyCollisionPredicate, IDumper, ITankEventListener, GameActionListener
    {
        ...
        private static const SNOW_DUST_DATA:Class = BattlefieldModel_SNOW_DUST_DATA;
        private static const DUST_DATA:Class = BattlefieldModel_DUST_DATA;

        private const temporaryDecals:Queue = new Queue();
        private const allDecals:Dictionary = new Dictionary();

        private var MAX_FRAMERATE:int = 60;
        ...
        public var screenSize:int = screenSizeSteps;
        private var plugins:Vector.<IBattlefieldPlugin>;
        private var pluginCount:int;
        private var border:ViewportBorder = new ViewportBorder();
        private var deltaSec:Number = 0;
        private var debugMode:Boolean;
        private var doRender:Boolean;
        private var physicsTimeStat:TimeStatistics = new TimeStatistics();
        private var logicTimeStat:TimeStatistics = new TimeStatistics();
        private var fullTimeStat:TimeStatistics = new TimeStatistics();
        private var lastError:Error;
        private var uiLockCount:int;
        private var cameraUnlockCounter:int;
        public var activeCameraController:*;
        ...
        private var cameraPosition:Vector3 = new Vector3();
        ...
        private var tankExplosionStartPosition:ConsoleVarInt = new ConsoleVarInt("tankexpl_soffset", 0, 0, 1000);
        private var tankExplosionVolume:ConsoleVarFloat = new ConsoleVarFloat("tankexpl_svolume", 0.4, 0, 1);
        ...
        private var lightings:Vector.<ToyLightAnimation> = new Vector.<ToyLightAnimation>();
        public var hidableObjects:HidableGraphicObjects = new HidableGraphicObjects();
        ...
        public var libs:PropLibRegistry = new PropLibRegistry();
        ...
        public function BattlefieldModel()
        {
            ...
            if (Screen)
            {
                this.MAX_FRAMERATE = (Screen.mainScreen.mode.refreshRate > 0) ? Screen.mainScreen.mode.refreshRate : 60;
            }
            // var controller:Controllers = new Controllers()
            // Main.cursorLayer.addChild(controller);
        ...
        public function settingsAccepted():void
        {
            ...
            if (settings != null)
            {
                ...
                trace("Dust enabled: " + settings.dust);
                this.gameMode.applyChangesBeforeSettings(settings);
                for (key in this.bfData.activeTanks)
                {
                    t = (key as TankData);
                    t.tank.setAnimationTracks(true);
                }
                this.animatedTracks = true;
                this.bfData.viewport.enableAmbientShadows(settings.shadowUnderTanks);
                // this.bfData.viewport.camera.view.antiAliasEnabled = SharedObject.getLocal("gtanks").data["antiAlias"];
                this.bfData.viewport.camera.view.antiAliasEnabled = true;
            }
        }
        ...
        public function addBonus(clientObject:ClientObject, bonusId:String, bonusObjectId:String, position:Vector3d, disappearingTime:int = 21, instant:Boolean = false):void
        {
            this.createBonusAndAttach(clientObject.register, bonusId, bonusObjectId, position, 0, true, clientObject, disappearingTime, instant);
        }
        ...
        private function attemptToTakeBonus(bfObject:ClientObject, bonusId:String):void
        {
            ...
            Logger.log("battle;attempt_to_take_bonus;" + JSON.stringify(json));
        }
        ...
        private function onResourcesReady(clientObject:ClientObject):void
        {
            ...
            this.bfData.viewport.camera.addToDebug(Debug.BONES, Object3D);
            this.bfData.viewport.camera.addToDebug(Debug.BOUNDS, Object3D);
            this.bfData.viewport.camera.addToDebug(Debug.EDGES, Object3D);
            this.bfData.viewport.camera.addToDebug(Debug.LIGHTS, Object3D);
            this.bfData.viewport.camera.addToDebug(Debug.NODES, Object3D);
            ...
            TankModel.MIN_ALLOWED_Z = this.bfData.viewport._mapContainer.boundMinZ - 500;
        }
        public function objectUnloaded(clientObject:ClientObject):void
        {
            var b:IBonus;
            var bgService:IBackgroundService;
            var o:* = undefined;
        ...
        {
            this.followCameraController.activate(this.bfData.viewport.camera);
        ...
        public function setCameraTarget(tank:Tank):void
        {
            ...
            this.followCameraController.setTarget(tank);
            this.followCameraController.activate(this.bfData.viewport.camera);
        }
        public function showSuicideIndicator(time:int):void
        {
            this.suicideIndicator.visible = false;
            this.suicideIndicator = new SuicideIndicator(this);
            ...
            this.suicideIndicator.show(3000);
        }
        ...
        public function addPlugin(plugin:IBattlefieldPlugin):void
        {
            ...
            {
                var _local_2:* = this.pluginCount++;
                this.plugins[_local_2] = plugin;
            }
        }
        ...
        private function initPhysicsAndViewport():void
        {
            ...
            if (this.gameMode == GameModes.SPACE)
            {
                ...
                {
                    if (this.mapResourceId == "map_satellite")
                    {
                        physics.gravity = new Vector3(0, 0, -450);
                    }
                    else
            ...
            {
                this.dusts.init(new SNOW_DUST_DATA().bitmapData, 4000, 2000, 80, 0.15, 0.29);
            }
            else
            {
                this.dusts.init(new DUST_DATA().bitmapData, 4000, 2000, 80, 0.15, 0.29);
            }
            this.suicideIndicator = new SuicideIndicator(this);
            this.suicideIndicator.visible = false;
            this.bfData.viewport.addChild(this.suicideIndicator);
            ...
            ResourceUtil.addEventListener(function():void
                {
                    ...
                    internal::mapResourceId = internal::mapResourceId;
                    config = new Config();
                    config.load("props.tara", internal::mapResourceId);
                });
        ...
        public function addDecal(param1:Vector3, param2:Vector3, param3:Number, param4:TextureMaterial, param5:RotationState = null, param6:Boolean = false):Decal
        {
            return this.bfData.viewport.addDecal(param1, param2, param3, param4, param5, param6);
        }
        ...
        private function createSkyBox(skyboxId:String):SkyBox
        {
            ...
            var skyBox:SkyBox = new SkyBox(SKYBOX_SIZE, new TextureMaterial(part1), new TextureMaterial(part2), new TextureMaterial(part3), new TextureMaterial(part4), new TextureMaterial(part5), new TextureMaterial(part6), 0);
            this.skyboxRevolver = new Object3DRevolver(skyBox, new Vector3(10, 3, 0), ((this.gameMode == GameModes.SPACE) ? 0.04 : 0));
            return (skyBox);
        }
        ...
        private function addMainListeners():void
        {
            ...
            this.bfData.guiContainer.addEventListener(Event.ENTER_FRAME, this.loop, false, 0, false);
        ...
        private function loop(e:Event):void
        {
            ...
            {
                this.activeCameraController.update(this.bfData.viewport.camera, this.bfData.time, deltaMsec);
            }
        ...
        {
            var spaceService:ISpaceService;
            var panelObjectId:String;
        ...
        private function updateTanks(time:int, deltaMillis:int, deltaSec:Number, t:Number):void
        {
            ...
            if (this.bfData != null && this.bfData.localUser != null)
            {
        ...
        private function playSpecialEffects(dt:int):void
        {
            ...
            for (key in this.bfData.graphicEffects)
            {
                effect = key;
        ...
        private function addTankToField(tankData:TankData):void
        {
            ...
            try
            {
                ...
                tankData.tank.setAnimationTracks(true);
        ...
        private function createBonusAndAttach(objectRegister:ObjectRegister, bonusId:String, bonusObjectId:String, position:Vector3d, livingTime:int, isFalling:Boolean, clientObject:ClientObject, disappearingTime:int = 21, instant:Boolean = false):IBonus
        {
            ...
            bonusModel.initObject(clientObject, ("bonus_box_" + bonusId.split("_")[0]), "cords", disappearingTime, "parachute_inner", "parachute", instant);
