package alternativa.tanks.models.battlefield.gui.chat
{
    ...
    public class BattleChatLine extends MessageLine
    {
        ...
        private var _nameFrom:Label = new Label();
        ...
        public function BattleChatLine(lineWidth:int, messageLabel:String, rank:int, chatPermissions:int, name:String, text:String, textColor:uint = 0xFFFFFF, premium:Boolean = false)
        {
            this.rankIcon = new SmallRankIcon(rank, premium);
            this.rankIcon.mouseEnabled = false;
            this.rankIcon.y = 3;
            this._rank = rank;
            this._width = lineWidth;
            this.output.color = 0xFFFFFF;
            ...
            if (messageLabel != null)
            {
                ...
                if (((chatPermissions > 0) && (!(this.chatLevel == null))))
                {
                    this.msgLabel.x = (this.msgLabel.x + ((!(this.chatLevel == null)) ? this.chatLevel.width : 0));
                }
                fx = (fx + ((!(this.chatLevel == null)) ? this.chatLevel.width : 0));
            }
            this.rankIcon.x = fx;
            fx = (fx + (this.rankIcon.width - 3));
            addChild(this._nameFrom);
            this._nameFrom.x = fx;
            fx = (fx + this._nameFrom.textWidth);
