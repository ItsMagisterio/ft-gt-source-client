package alternativa.tanks.models.battlefield.gui.chat
{
    ...
    public class ChatModel extends ChatModelBase implements IChatModelBase, IObjectLoadListener, IPanelListener, IChatBattle
    {
        ...
        public function addMessage(clientObject:ClientObject, userId:Long, message:String, _arg_4:BattleTeamType, teamOnly:Boolean, nick:String = null, rank:int = 0, chat_level:int = 0, premium:Boolean = false):void
        {
            var messageLabel:String = (((!(_arg_4 == BattleTeamType.NONE)) && (teamOnly)) ? "[TEAM]" : null);
            var userName:String = nick;
            var userRank:int = rank;
            this.battleChat.addUserMessage(messageLabel, userName, userRank, chat_level, _arg_4, (message + "\n"), premium);
        }
...
class ExpectingMessage
{
    ...
    public function ExpectingMessage(messageLabel:String, userID:Long, _arg_3:BattleTeamType, message:String)
    {
