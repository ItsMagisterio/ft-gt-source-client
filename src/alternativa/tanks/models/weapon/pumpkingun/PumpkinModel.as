package alternativa.tanks.models.weapon.pumpkingun
{
    ...
    public class PumpkinModel extends RicochetModelBase implements IRicochetModelBase, IObjectLoadListener, IWeaponController, IPumpkinShotListener
    {
        ...
        private var readyTime:SecureInt = new SecureInt("readyTime.value ricochet");
        private var currentEnergy:SecureNumber = new SecureNumber("currentEnergy.value ricochet");
        ...
        private var _shotDirection:Vector3 = new Vector3();
        private var _localPosition:Vector3 = new Vector3();
        private var _hitDirection:Vector3 = new Vector3();
        ...
        public function update(time:int, deltaTime:int):Number
        {
            ...
            {
                this.currentEnergy.value = (this.currentEnergy.value + ((this.localPumpkinData.energyRechargeSpeed * deltaTime) * 0.001));
                if (this.currentEnergy.value > energyCapacity)
                {
        ...
        private function doFire(param1:WeaponCommonData):void
        {
            ...
            var _loc4_:Vector3;
            var _loc5_:Vector3d;
            var _loc6_:PumpkinShot;
            this.readyTime.value = (getTimer() + this.localShotData.reloadMsec.value);
            this.currentEnergy.value = (this.currentEnergy.value - this.localPumpkinData.energyPerShot);
            ...
            {
                _loc3_ = int((((this.rayHit.t / _loc2_.y) * 0xFF) - 128));
            ...
            {
                if (this.battlefield.getBattlefieldData().collisionDetector.intersectRayWithStatic(this.localTankData.tank.state.pos, this._barrelOrigin.vClone().vRemove(this.localTankData.tank.state.pos).vNormalize(), CollisionGroup.STATIC, _loc2_.y, null, this.rayHit))
                {
                    _loc3_ = int((((this.rayHit.t / _loc2_.y) * 0xFF) - 128));
                    _loc4_ = this.localTankData.tank.state.pos;
                    _loc5_ = new Vector3d(_loc4_.x, _loc4_.y, _loc4_.z);
        ...
        private function createSelfHitEffects(shooterData:TankData, ricochetSFXData:PumpkingunSFXData, weaponCommonData:WeaponCommonData, relativeHitPos:int, globalBarrelOrigin:Vector3, gunDirection:Vector3):void
        {
            var sound:Sound3D;
            var soundEffect:Sound3DEffect;
            var offsetAlongBarrel:Number = (((relativeHitPos + 128) / 0xFF) * weaponCommonData.muzzles[0].y);
