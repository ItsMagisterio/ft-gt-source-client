package
{
   ...
   import logic.server.models.inventory.ServerInventoryModel;
   import logic.server.models.mines.ServerBattleMinesModel;
   ...
   import logic.server.models.inventory.ServerInventoryData;
   import logic.gui.AlertBugWindow;
   ...
   import logic.gui.ServerMessage;
   ...
   import alternativa.tanks.input.BattleInput;
   ...
   public class BattleController implements INetworkListener
   {
      ...
      private var keysBindingService:KeysBindingServiceImpl;
      private var battleInputService:BattleInputServiceImpl;
      private var fullscreenService:FullscreenServiceImpl;

      public function BattleController()
      {
         this.keysBindingService = Main.osgi.getService(KeysBindingService) as KeysBindingServiceImpl;
         this.fullscreenService = Main.osgi.getService(FullscreenService) as FullscreenServiceImpl;
         this.battleInputService = new BattleInputServiceImpl();
         Main.osgi.registerService(BattleInputService, battleInputService);
         ...
         Main.osgi.registerService(FullscreenService, fullscreenService);
      }
      public static function getWeaponController(obj:ClientObject, tankOwner:ClientObject):IWeaponController
      {
         var weaponController:IWeaponController;
         var id:String = obj.id.split("_")[0];
         var turrEntity:* = WeaponsManager.specialEntity[obj.id]; // NULL
         switch (id)
         {
            case "railgun":
            ...
            case "isida":
               ...
               isidaModel.initObject(WeaponsManager.getObjectFor(obj.id), turrEntity.angle, turrEntity.capacity, turrEntity.chargeRate, turrEntity.tickPeriod, turrEntity.coneAngle, turrEntity.dischargeRate, turrEntity.radius, tankOwner);
            ...
            case "thunder":
               ...
               thunderModel.initObject(WeaponsManager.getObjectFor(obj.id), turrEntity.impactForce, turrEntity.maxSplashDamageRadius, turrEntity.minSplashDamagePercent, turrEntity.minSplashDamageRadius, tankOwner);
            ...
            case "thunder_xt":
               ...
               thunderXTModel.initObject(WeaponsManager.getObjectFor(obj.id), turrEntity.impactForce, turrEntity.maxSplashDamageRadius, turrEntity.minSplashDamagePercent, turrEntity.minSplashDamageRadius, tankOwner);
            ...
            case "frezee":
               ...
               frezeeModel.initObject(WeaponsManager.getObjectFor(obj.id), turrEntity.damageAreaConeAngle, turrEntity.damageAreaRange, turrEntity.energyCapacity, turrEntity.energyDischargeSpeed, turrEntity.energyRechargeSpeed, turrEntity.weaponTickMsec, tankOwner);
            ...
            case "frezeeny":
               ...
               frezeeNYModel.initObject(WeaponsManager.getObjectFor(obj.id), turrEntity.damageAreaConeAngle, turrEntity.damageAreaRange, turrEntity.energyCapacity, turrEntity.energyDischargeSpeed, turrEntity.energyRechargeSpeed, turrEntity.weaponTickMsec, tankOwner);
            ...
            case "ricochet":
               ...
               ricochetModel.initObject(WeaponsManager.getObjectFor(obj.id), turrEntity.energyCapacity, turrEntity.energyPerShot, turrEntity.energyRechargeSpeed, turrEntity.shotDistance, turrEntity.shotRadius, turrEntity.shotSpeed, tankOwner);
      ...
      public function onData(data:Command):void
      {
            ...
            switch (data.type)
            {
               case Type.BATTLE:
                  ...
                  if (data.args[0] == "init_gui_model")
                  {
                     ...
                     {
                        users[i] = new UserStat(0, 0, obj.nickname, obj.rank, 0, 0, BattleTeamType.getType(obj.teamType), obj.nickname, obj.weapon, obj.isPremium);
                        i++;
                     }
                  ...
                  if (data.args[0] == "init_tank")
                  {
                     ...
                     Logger.log("init_tank: " + data.args[1]);
                     break;
                  }
                  if (data.args[0] == "activate_tank")
                  {
                     ...
                     {
                        Logger.log("activate_tank: " + data.args[1]);
                  ...
                  if (data.args[0] != "tracePing")
                  {
                     ...
                     if (data.args[0] == "spawn_bonus_instant")
                     {
                        this.parseSpawnBonus(data.args[1], true);
                        break;
                     }
                     ...
                     {
                        this.createDamageEffect(activeTanks[data.args[1]], data.args[2], int(data.args[3]));
                        break;
                     }
                     ...
                     if (data.args[0] == "kick_for_cheats")
                     {
                        ...
                        alertWindow._msg = "Вы были кикнуты за читы.";
                     ...
                     if (data.args[0] == "init_dom_model")
                     {
                        ...
                        for each (obj in JSON.parse(data.args[1]).points)
                        {
                           ...
                           point.id = TextUtils.getIntByAlphabet(obj.id);
                     ...
                     if (data.args[0] == "remove_graffiti")
                     {
                        this.removeGraffiti(data.args[1]);
                        break;
                     }
                     if (data.args[0] == "init_inventory")
                     {
                        // if(this.serverInventoryModel != null){
                        // if(this.serverInventoryModel.inventoryModel != null){
                        // this.serverInventoryModel.inventoryModel.objectUnloaded(null);
                        // break;
                        // }
                        // }
                     ...
                     {
                        DOMModel(Main.osgi.getService(IDOMModel)).serverTankCapturingPoint(TextUtils.getIntByAlphabet(data.args[1]), activeTanks[data.args[2]]);
                        break;
                     }
                     if (data.args[0] == "tank_leave_capturing_point")
                     {
                        DOMModel(Main.osgi.getService(IDOMModel)).serverTankLeaveCapturingPoint(activeTanks[data.args[1]], TextUtils.getIntByAlphabet(data.args[2]));
                        break;
                     }
                     if (data.args[0] == "disable_tanks_capturing_points")
                     {
                        DOMModel(Main.osgi.getService(IDOMModel)).serverRemoveAllCapturingPoints();
                        break;
                     }
                     if (data.args[0] == "set_point_score")
                     {
                        DOMModel(Main.osgi.getService(IDOMModel)).serverSetPointScore(TextUtils.getIntByAlphabet(data.args[1]), data.args[2]);
                        break;
                     }
                     if (data.args[0] == "point_captured_by")
                     {
                        DOMModel(Main.osgi.getService(IDOMModel)).serverPointCapturedBy(data.args[1], TextUtils.getIntByAlphabet(data.args[2]));
                        break;
                     }
                     if (data.args[0] == "point_lost_by")
                     {
                        DOMModel(Main.osgi.getService(IDOMModel)).serverPointLostBy(data.args[1], TextUtils.getIntByAlphabet(data.args[2]));
                        break;
                     }
                     ...
                     if (data.args[0] == "res_user")
                     {
                        Main.openingToGarage = true;
                        resUser(data.args[1]);
                        break;
                     }
                     if (data.args[0] == "res1_user")
                     {
                        resUser1(data.args[1]);
                        break;
                     }
                     if (data.args[0] == "init_suicide_garage")
                     {
                        PanelModel(Main.osgi.getService(IPanel)).garageSelf();
                        break;
                     }
                     if (data.args[0] == "close_garage")
                     {
                        PanelModel(Main.osgi.getService(IPanel)).closeGarage();
                        break;
                     }
                     break;
                  }
         ...
         catch (e:Error)
         {
            ...
            // alert.text = "An error occurred: " + e.getStackTrace();
            // Main.systemUILayer.addChild(alert);
            throw e;
         }
      }

      private function createDamageEffect(user:ClientObject, damage:int, type:int):void
      {
         var isEnable:Boolean = BattlefieldModel(Main.osgi.getService(IBattleField)).isEnableDamageUpEffect();
         if (isEnable)
         {
            new DamageEffect().createEffect(TankModel(Main.osgi.getService(ITank)).getTankData((activeTanks[user.id] as ClientObject)).tank, Math.floor(damage), type);
         }
      }
      private function createGraffiti(name:String, json:String):void
      {
         ...
         var origin:Vector3 = new Vector3(parser.origin.x, parser.origin.y, parser.origin.z + 2000);
         var decalId:String = parser.id;
         GraffitiManager.createGraffiti(rayPos, name, decalId);
      }
      ...
      private function resUser1(js:String):void
      {
         ...
         tankParts.turretSkinId = json.turret_skin;
         tankParts.shotEffectId = json.shot_effect;
         ...
         clientTank.spawnState = TankSpawnState.NEWCOME;
         ...
         tankModelService.initObject(activeTanks[json.tank_id], json.battleId, json.mass, json.power, null, tankParts, null, json.impact_force, json.kickback, json.turret_rotation_accel, json.turret_turn_speed, json.nickname, json.rank, tankParts.turretObjectId, tankParts.turretSkinId, tankParts.shotEffectId, json.isPremium);
         tankModelService.initTank(activeTanks[json.tank_id], clientTank, tankParts, localTankInited, true);
         StatisticsModel(Main.osgi.getService(IBattlefieldGUI)).logUserAction(json.tank_id, "has changed equipment");
      }
      private function removeGraffiti(id:String):void
      {
         GraffitiManager.removeGraffiti(id);
      }
      ...
      private function parseFinishBattle(json:String):void
      {
         ...
         for each (obj in parser.users)
         {
            stat = new UserStat(obj.kills, obj.deaths, obj.id, obj.rank, obj.score, obj.prize, BattleTeamType.getType(obj.team_type), obj.id, obj.weapon, obj.isPremium);
            users.push(stat);
         }
      ...
      {
         var parser:Object = JSON.parse(json);
         var stat:UserStat = new UserStat(parser.kills, parser.deaths, parser.id, parser.rank, parser.score, 0, BattleTeamType.getType(parser.team_type), parser.id, parser.weapon, parser.isPremium);
      ...
      private function parseStopFire(user:ClientObject, firing:String):void
      {
         ...
         if (activeTanks[user.id] == null)
         {
            return;
         }
         var flamethrower:FlamethrowerModel;
      ...
      {
         var id:String;
         var models:IModelService;
         id = null;
         var railgun:RailgunModel;
         var flamethrower:FlamethrowerModel;
         var action:IsisAction;
         var parser:Object;
      ...
      private function parseFire(user:ClientObject, js:String):void
      {
         ...
         var hitPoints:Array;
         var targets:Array;
         var targetPostitions:Array;
         var i:int = 0;
         var railgun_xt:RailgunModel;
         var railgun_cs:RailgunModel;
         var doubleShot:Boolean;
         var terminator:TerminatorModel;
         parser = null;
         td = null;
         targetInc = null;
         hitPoints = null;
         ...
         var railgun:RailgunModel;
         var hitPoint:Vector3d;
         var smoky:SmokyModel;
         var hitPos:Vector3d;
         var shaftModel:ShaftModel;
         var ids:Array;
         var static_point:Vector3;
         i = 0;
         var obj:Object;
         var objj:Object;
         parser = null;
         ...
         var userObject:ClientObject = td.user;
         if (td == null || td.turret == null || td.turret.id == null)
         {
            return;
         }
         ...
         switch (id)
         {
            case "railgun":
               ...
               railgun = getWeaponController(td.turret, userObject) as RailgunModel;
               if (!railgun)
               {
                  return;
               }
            ...
            case "railgun_xt":
               ...
               railgun_xt = getWeaponController(td.turret, userObject) as RailgunModel;
               if (!railgun_xt)
               {
                  return;
               }
            ...
            case "railgun_cs":
               ...
               railgun_cs = getWeaponController(td.turret, userObject) as RailgunModel;
               if (!railgun_cs)
               {
                  return;
               }
            ...
            case "terminator":
               ...
               terminator = getWeaponController(td.turret, userObject) as TerminatorModel;
               if (!terminator)
               {
                  return;
               }
            ...
            case "smoky":
               ...
               smoky = getWeaponController(td.turret, userObject) as SmokyModel;
               if (!smoky)
               {
                  return;
               }
            ...
            case "hwthunder":
               ...
               break;
            case "shaft":
               shaftModel = getWeaponController(td.turret, userObject) as ShaftModel;
               if (!shaftModel)
               {
                  return;
               }
               ...
               {
                  ids.push(new ServerShaftTargetData(new Vector3(obj.pos.x, obj.pos.y, obj.pos.z), new Vector3(obj.point_pos.x, obj.point_pos.y, obj.point_pos.z), obj.id));
               }
      ...
      private function parseSpawnBonus(js:String, instant:Boolean = false):void
      {
         var parser:Object = JSON.parse(js);
         ...
         BattlefieldModel(Main.osgi.getService(IBattleField)).addBonus(bonus_obj, parser.id, parser.id, new Vector3d(parser.x, parser.y, parser.z), parser.disappearing_time, instant);
      }
      ...
      private function onChatMessage(json:String):void
      {
         ...
         if ((!(parser.system)))
         {
            ChatModel(Main.osgi.getService(IChatBattle)).addMessage(null, null, parser.message, BattleTeamType.getType(parser.team_type), parser.team, parser.nickname, parser.rank, parser.chat_level, parser.isPremium);
         }
         else
         {
      ...
      private function initBattle(js:String):void
      {
         ...
         if (IStorageService(Main.osgi.getService(IStorageService)).getStorage().data["night_mode"] != null)
         {
         ...
         this.battle.bm.bfData.bfObject.putParams(BattleOptions, json);
      }
