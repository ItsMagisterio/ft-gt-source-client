package forms.zad
{
   ...
   public class PartsList extends Sprite
   {
	  ...
	  public function addItem(id:Object, name:String, type:int, sort:int, count:int, preview:String, id1:Object, name1:String, type1:int, sort1:int, count1:int, preview1:String) : void
      {
			 ...
			 data1.par = parts;
			 ++parts;
          //trace("name: "+name+" type: "+type+" sort: "+sort+" count: "+count+" preview: "+preview+" name1: "+name1+" type1: "+sort1+" count1: "+count1+" preview: "+preview1);
          //new method get the resource directly from the image resources
         if (ResourceUtil.getResource(ResourceType.IMAGE, preview + "_preview") == null){
            if (ResourceUtil.getResource(ResourceType.IMAGE, preview) != null){
               data.preview = (ResourceUtil.getResource(ResourceType.IMAGE, (preview)) as ImageResource).bitmapData;
            }
         }else{
            if (ResourceUtil.getResource(ResourceType.IMAGE, preview + "_preview") != null){
               data.preview = (ResourceUtil.getResource(ResourceType.IMAGE, (preview + "_preview")) as ImageResource).bitmapData;
            }
         }

         if (ResourceUtil.getResource(ResourceType.IMAGE, preview1 + "_preview") == null){
            if (ResourceUtil.getResource(ResourceType.IMAGE, preview1) != null){
               data1.preview = (ResourceUtil.getResource(ResourceType.IMAGE, (preview1)) as ImageResource).bitmapData;
            }
         }else{
            if (ResourceUtil.getResource(ResourceType.IMAGE, preview1 + "_preview") != null){
               data1.preview = (ResourceUtil.getResource(ResourceType.IMAGE, (preview1 + "_preview")) as ImageResource).bitmapData;
            }
         }
         addI(data,data1);
      }
