package forms.zad 
{
	...
	public class Challenge extends Sprite
	{
		...
		private var part:int = 0;//current level
		private var mpart:int = 0;//maximum level
		private var pro:int = 0;//scores
		private var mpro:int = 0;//points to the next level
		private var mpr:int = 0;//next level completion percentage
		...
		public function inf(zvc:int,mzvc:int,p:int,ctSta:int,full:int) : void
		  {
			if(zvc >= full){
				prl.text = "Stars " + zvc + "/" + full;
				tprl.text = String(int(p - 1)) + "";
				pr.redr(ChProgress.MAX_WIDTH * (1/1));
			}else{
				prl.text = "Stars " + zvc + "/" + mzvc;
				tprl.text = p + "";
				var currentTierProgress:int = zvc - ctSta;
				var nextTierStarsRequired:int = mzvc - ctSta;
				pr.redr(ChProgress.MAX_WIDTH * (currentTierProgress/nextTierStarsRequired));
			}
		  }
		  ...
		  private function showCountDownTick(e:TimerEvent = null) : void
{
    ...
    var day:int = int(this.countDown / 86400);
    var cho:int = int((this.countDown - (day * 86400)) / 3600);
    var min:int = int((this.countDown - (cho * 3600)) / 60); // Corrected calculation for minutes
    var sec:int = int(this.countDown % 60); // Corrected calculation for seconds
