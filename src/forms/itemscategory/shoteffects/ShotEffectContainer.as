package forms.itemscategory.shoteffects
{
	...
	public class ShotEffectContainer extends Sprite
	{
		...
		public function ShotEffectContainer(skinName:String, skinDescription:String, skinType:String, equip:Boolean, price:int, bought:Boolean)
		{
		...
		{
			Network(Main.osgi.getService(INetworker)).send("garage;equip_shot_effect;" + skinType);
		}
		...
		{
			if (skinIcon == null || skinIcon == "" || skinIcon == "default")
			{
				iconToShow = new Bitmap(ResourceUtil.getResource(ResourceType.IMAGE, "standard").bitmapData);
			}
			else
			{
				iconToShow = new Bitmap(ResourceUtil.getResource(ResourceType.IMAGE, skinIcon).bitmapData);
			}
