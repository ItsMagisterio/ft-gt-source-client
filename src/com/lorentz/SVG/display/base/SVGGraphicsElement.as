package com.lorentz.SVG.display.base
{
    ...
    public class SVGGraphicsElement extends SVGElement
    {
        ...
        {
            if ((!(this._renderInvalidFlag)))
            {
                this._renderInvalidFlag = true;
        ...
        {
            var length:String;
            var dashAlign:String;
            if ((!(this.hasDashedStroke)))
            {
                return;
            }
            var strokeDashArray:Array = [];
            for each (length in SVGParserCommon.splitNumericArgs(finalStyle.getPropertyValue("stroke-dasharray")))
            {
            ...
            dashAlign = String(((finalStyle.getPropertyValue("stroke-dashalign")) || ("none"))).toLowerCase();
        ...
        protected function beginFill(g:Graphics = null, callBack:Function = null):void
        {
            ...
            var rgrad:SVGRadialGradient;
            var patternValidated:Function;
            var color:uint;
            if (this.hasFill)
            {
                fill = finalStyle.getPropertyValue("fill");
                fillOpacity = Number(((finalStyle.getPropertyValue("fill-opacity")) || (1)));
                if (fill == null)
                {
                ...
                else
                {
                    ...
                    {
                        id = SVGUtil.extractUrlId(fill);
                        grad = (document.getDefinition(id) as SVGGradient);
                        if (grad != null)
                        {
                            switch (grad.type)
                            {
                                case GradientType.LINEAR:
                                    this.doLinearGradient((grad as SVGLinearGradient), g, true);
                                    break;
                                case GradientType.RADIAL:
                                    rgrad = (grad as SVGRadialGradient);
                                    if (rgrad.r == "0")
                                    {
                                    ...
                                    {
                                        this.doRadialGradient(rgrad, g, true);
                                    }
                                    break;
                            }
                            if (callBack != null)
                        ...
                        pattern = (document.getDefinitionClone(id) as SVGPattern);
                        if (pattern)
                        {
                            attachElement(pattern);
                            patternValidated = function(e:Event):void
                            {
                    ...
                    {
                        color = SVGColorUtils.parseToUint(fill);
        ...
        protected function lineStyle(g:Graphics):void
        {
            ...
            if (this.hasStroke)
            {
                strokeOpacity = Number(((finalStyle.getPropertyValue("stroke-opacity")) || (1)));
                strokeWidth = 1;
                ...
                strokeLineJoin = JointStyle.MITER;
                ...
                strokeMiterlimit = Number(((finalStyle.getPropertyValue("stroke-miterlimit")) || (4)));
                stroke = finalStyle.getPropertyValue("stroke");
                color = SVGColorUtils.parseToUint(stroke);
                ...
                {
                    id = SVGUtil.extractUrlId(stroke);
                    grad = (document.getDefinition(id) as SVGGradient);
                    if (grad != null)
                    {
                        switch (grad.type)
                        {
                            case GradientType.LINEAR:
                                this.doLinearGradient((grad as SVGLinearGradient), g, false);
                                break;
                            case GradientType.RADIAL:
                                rgrad = (grad as SVGRadialGradient);
                                if (rgrad.r == "0")
                                ...
                                {
                                    this.doRadialGradient(rgrad, g, false);
                                }
                                break;
                        }
                    }
