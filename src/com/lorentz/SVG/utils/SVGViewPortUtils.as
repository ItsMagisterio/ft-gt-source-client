package com.lorentz.SVG.utils
{
    ...
    public class SVGViewPortUtils
    {
        ...
        public static function getContentMetrics(viewPortRect:Rectangle, contentBox:Rectangle, contentAlign:String, meetOrSlice:String):Object
        {
            ...
            {
                case "xmid":
                    x = (x + ((viewPortRect.width / 2) - ((contentBox.width * scaleX) / 2)));
                    break;
                case "xmax":
                    x = (x + (viewPortRect.width - (contentBox.width * scaleX)));
                    break;
            }
            switch (yPart)
            {
                case "ymid":
                    y = (y + ((viewPortRect.height / 2) - ((contentBox.height * scaleY) / 2)));
                    break;
                case "ymax":
                    y = (y + (viewPortRect.height - (contentBox.height * scaleY)));
                    break;
            }
            return ( {
