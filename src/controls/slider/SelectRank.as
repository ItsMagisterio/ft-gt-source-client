package controls.slider
{
    ...
    public class SelectRank extends Slider
    {
        ...
        protected function checkMinRang():void
        {
            ...
            {
                if (this._minRang < (this._currentRang - this._maxRangRange))
                {
                ...
                {
                    if (this._minRang < (this._maxRang - this._maxRangRange))
                    {
        ...
        protected function checkMaxRang():void
        {
            ...
            {
                if (this._maxRang > (this._currentRang + this._maxRangRange))
                {
                ...
                {
                    if (this._maxRang > (this._minRang + this._maxRangRange))
                    {
        ...
        override protected function dragThumb(e:MouseEvent):void
        {
            ...
            {
                this._minRang = (this._minRang + int(((this.sthumb.leftDrag.mouseX - curThumbX) / this._thumbTick)));
                this.checkMinRang();
                this.checkMaxRang();
            }
            else
            {
                if (((trgt == this.sthumb.rightDrag) && ((rightFlag) || (minMaxRangFlag))))
                {
                    this._maxRang = (this._maxRang + int(((this.sthumb.rightDrag.mouseX - curThumbX) / this._thumbTick)));
                    this.checkMinRang();
                    this.checkMaxRang();
                }
                else
                {
                    if (((trgt == this.sthumb.centerDrag) && (((leftFlag) || (maxMinRangFlag)) && ((rightFlag) || (minMaxRangFlag)))))
                    {
                        this._minRang = (this._minRang + int(((this.sthumb.centerDrag.mouseX - curThumbX) / this._thumbTick)));
                        this._maxRang = (this._maxRang + int(((this.sthumb.centerDrag.mouseX - curThumbX) / this._thumbTick)));
                        if (this._minRang < _minValue)
                        {
